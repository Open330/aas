...
use crate::adapters::util::{
    chat_messages_from_common, chat_messages_to_common, chat_tools_from_common, chat_tools_to_common, parse_chat_tool_deltas, sse_data as sse, sse_headers, to_text,
};
use crate::models::{resolve_choice, BackendChoice};
use crate::types::{
    AgentAdapter, BackendAdapter, CommonEvent, CommonRequest, CommonResponse, StreamCtx, UpstreamRequest,
};
...
impl AgentAdapter for GrokAgent {
    fn parse_request(&self, _path: &str, body: &Value) -> CommonRequest {
        ...
        let msgs = body.get("messages").and_then(|v| v.as_array()).unwrap_or(&empty);
        ...
        let non_system: Vec<Value> = msgs
            ...
            .collect();
        CommonRequest {
            model: body.get("model").and_then(|v| v.as_str()).unwrap_or("asx-proxy").to_string(),
            system,
        ...
        match ev {
            ...
            CommonEvent::Done { finish_reason } => {
                let reason = if !ctx.items.is_empty() {
                    ...
                    finish_reason.clone().filter(|s| !s.is_empty()).unwrap_or_else(|| "stop".to_string())
                };
    ...
    fn format_response(&self, resp: &CommonResponse, req: &CommonRequest) -> Value {
        ...
        let finish = if !resp.tool_calls.is_empty() {
            ...
            resp.finish_reason.clone().filter(|s| !s.is_empty()).unwrap_or_else(|| "stop".to_string())
        };
        json!({
            "id": "chatcmpl-asx",
    ...
    fn format_models(&self, choices: &[BackendChoice]) -> Value {
        let data: Vec<Value> = choices
            .iter()
            .map(|c| json!({ "id": c.id, "object": "model", "created": 0, "owned_by": "asx-proxy" }))
            .collect();
...
impl BackendAdapter for GrokBackend {
    fn build_request(&self, req: &CommonRequest, cred: &str) -> UpstreamRequest {
        ...
        UpstreamRequest {
            ...
            headers: vec![
                ...
                ("Authorization".to_string(), format!("Bearer {}", grok_token(cred))),
                ("X-XAI-Token-Auth".to_string(), "xai-grok-cli".to_string()),
                ("x-grok-client-version".to_string(), GROK_VERSION.to_string()),
                ("x-grok-client-identifier".to_string(), "grok-shell".to_string()),
                ("User-Agent".to_string(), format!("grok-shell/{GROK_VERSION} (macos; aarch64)")),
    ...
    fn parse_stream_chunk(&self, block: &str) -> Vec<CommonEvent> {
        ...
        for line in block.split('\n') {
            ...
            let Some(ch) = j.get("choices").and_then(|v| v.as_array()).and_then(|a| a.first()) else { continue };
            if let Some(text) = ch.get("delta").and_then(|d| d.get("content")).and_then(|v| v.as_str()) {
                if !text.is_empty() {
                    out.push(CommonEvent::Text { text: text.to_string() });
                }
            }
            if let Some(tcs) = ch.get("delta").and_then(|d| d.get("tool_calls")).and_then(|v| v.as_array()) {
                out.extend(parse_chat_tool_deltas(tcs));
            }
            if let Some(fr) = ch.get("finish_reason").and_then(|v| v.as_str()) {
                out.push(CommonEvent::Done { finish_reason: Some(fr.to_string()) });
            }
        }
        out
    }
