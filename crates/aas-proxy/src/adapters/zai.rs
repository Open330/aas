...
use crate::adapters::util::{chat_messages_from_common, chat_tools_from_common, parse_chat_tool_deltas};
...
pub fn is_zai_overload(body: &str) -> bool {
    ...
    if ZAI_RETRY_CODES.iter().any(|c| body.contains(&format!("\"{c}\""))) {
        return true;
    }
    let lower = body.to_lowercase();
    lower.contains("overload") || lower.contains("try again later") || lower.contains("rate limit") || lower.contains("too many requests")
}
...
impl BackendAdapter for ZaiBackend {
    ...
    fn build_request(&self, req: &CommonRequest, cred: &str) -> UpstreamRequest {
        ...
        let effort = choice.effort.clone().or_else(|| req.reasoning_effort.clone());
        if let Some(e) = effort {
            let ty = if e == "none" || e == "off" { "disabled" } else { "enabled" };
    ...
    fn parse_stream_chunk(&self, block: &str) -> Vec<CommonEvent> {
        ...
        for line in block.split('\n') {
            ...
            let ch = j.get("choices").and_then(|v| v.as_array()).and_then(|a| a.first());
            let text = ch
            ...
            if let Some(t) = text {
                if !t.is_empty() {
                    out.push(CommonEvent::Text { text: t.to_string() });
                }
            }
            let tool_calls = ch
            ...
            if let Some(fr) = ch.and_then(|c| c.get("finish_reason")).and_then(|v| v.as_str()) {
